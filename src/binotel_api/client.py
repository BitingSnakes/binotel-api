"""Synchronous Binotel HTTP client."""

from __future__ import annotations

import json
import random
import threading
import time
from collections.abc import Callable
from dataclasses import dataclass
from datetime import timedelta
from typing import Any, Protocol, TypeVar

import wreq
from wreq.blocking import Client as WreqClient

from .config import BinotelConfig
from .exceptions import BinotelRequestError
from .resources import Calls, Customers, Settings, Stats

T = TypeVar("T")


class HttpResponse(Protocol):
    status: Any

    def json(self) -> Any: ...


class HttpClient(Protocol):
    def post(self, url: str, **kwargs: Any) -> HttpResponse: ...

    def close(self) -> None: ...


_RETRYABLE_WREQ_ERRORS = (
    wreq.ConnectionError,
    wreq.ConnectionResetError,
    wreq.ProxyConnectionError,
    wreq.RequestError,
    wreq.TimeoutError,
    wreq.TlsError,
)

_NON_RETRYABLE_WREQ_ERRORS = (
    wreq.BodyError,
    wreq.BuilderError,
    wreq.RedirectError,
    wreq.StatusError,
    wreq.UpgradeError,
)


@dataclass(slots=True)
class _CacheEntry:
    value: Any
    expires_at: float | None


class _MemoryCache:
    def __init__(self) -> None:
        self._items: dict[str, _CacheEntry] = {}
        self._lock = threading.Lock()

    def get_or_set(self, key: str, ttl: int, factory: Callable[[], T]) -> T:
        now = time.monotonic()
        with self._lock:
            entry = self._items.get(key)
            if entry is not None and (entry.expires_at is None or entry.expires_at > now):
                return entry.value

        value = factory()
        expires_at = None if ttl == -1 else now + ttl
        with self._lock:
            self._items[key] = _CacheEntry(value, expires_at)
        return value

    def clear(self) -> None:
        with self._lock:
            self._items.clear()


class BinotelClient:
    """Client exposing customers, stats, settings, and calls resources."""

    def __init__(
        self,
        config: BinotelConfig | None = None,
        *,
        http_client: HttpClient | None = None,
    ) -> None:
        self.config = config or BinotelConfig.from_env()
        self._base_url = self.config.url.rstrip("/")
        self._http: HttpClient = http_client or WreqClient(
            timeout=timedelta(seconds=self.config.timeout),
            connect_timeout=timedelta(seconds=self.config.connect_timeout),
            headers={"Accept": "application/json"},
        )
        self._cache = _MemoryCache()
        self._request_lock = threading.Lock()
        self._last_request_at: float | None = None

        self.customers = Customers(self)
        self.stats = Stats(self)
        self.settings = Settings(self)
        self.calls = Calls(self)

    def request(
        self,
        method: str,
        params: dict[str, Any] | None = None,
        *,
        response_key: str | None = None,
        cache_seconds: int | None = None,
    ) -> Any:
        params = dict(params or {})
        cache_key = self._cache_key(method, params)

        def make_request() -> Any:
            result = self._post(method, params)
            if response_key is None:
                return result
            if not isinstance(result, dict):
                return None
            return result.get(response_key)

        if cache_seconds is not None:
            return self._cache.get_or_set(cache_key, cache_seconds, make_request)
        return make_request()

    def _post(self, method: str, params: dict[str, Any]) -> Any:
        payload = {**params, "key": self.config.key, "secret": self.config.secret}
        url = f"{self._base_url}/{self.config.version}/{method}.{self.config.response_format}"
        attempts = max(1, self.config.retry_times)

        for attempt in range(1, attempts + 1):
            self._throttle()
            try:
                response = self._http.post(url, json=payload)
                status = self._status_code(response)
                if status == 429 or status >= 500:
                    if attempt < attempts:
                        self._retry_sleep(attempt)
                        continue
                if status >= 400:
                    raise BinotelRequestError(f"API request failed with HTTP status {status}")
                return response.json()
            except _RETRYABLE_WREQ_ERRORS as exc:
                if attempt < attempts:
                    self._retry_sleep(attempt)
                    continue
                raise BinotelRequestError(f"API request failed: {exc}") from exc
            except wreq.DecodingError as exc:
                raise BinotelRequestError("API returned invalid JSON") from exc
            except _NON_RETRYABLE_WREQ_ERRORS as exc:
                raise BinotelRequestError(f"API request failed: {exc}") from exc
            except (ValueError, TypeError) as exc:
                raise BinotelRequestError("API returned invalid JSON") from exc
            finally:
                with self._request_lock:
                    self._last_request_at = time.monotonic()

        raise BinotelRequestError("API request failed after all retries")

    def _throttle(self) -> None:
        gap = self.config.throttle_ms / 1_000
        if gap <= 0:
            return
        with self._request_lock:
            if self._last_request_at is None:
                return
            remaining = gap - (time.monotonic() - self._last_request_at)
        if remaining > 0:
            time.sleep(remaining)

    def _retry_sleep(self, attempt: int) -> None:
        base = min(
            self.config.retry_sleep_ms * (2 ** (attempt - 1)),
            self.config.retry_max_sleep_ms,
        )
        jitter = random.randint(0, max(1, int(base * 0.1)))
        time.sleep((base + jitter) / 1_000)

    @staticmethod
    def _cache_key(method: str, params: dict[str, Any]) -> str:
        return method + ":" + json.dumps(params, sort_keys=True, separators=(",", ":"))

    @staticmethod
    def _status_code(response: HttpResponse) -> int:
        status = response.status
        if isinstance(status, int):
            return status
        return status.as_int()

    def clear_cache(self) -> None:
        self._cache.clear()

    def close(self) -> None:
        self._http.close()

    def __enter__(self) -> BinotelClient:
        return self

    def __exit__(self, *_: object) -> None:
        self.close()


class Binotel(BinotelClient):
    """Short compatibility name mirroring the original package's entry point."""
